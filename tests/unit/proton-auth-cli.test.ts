...
import { buildProtonAuthEnv, resolveProtonAuthBinary } from '../../src/auth/login/proton-auth-cli.js';
...
describe('resolveProtonAuthBinary', () => {
  it('returns the path unchanged when it already exists or is not Windows', () => {
    expect(resolveProtonAuthBinary('/no/such/proton-auth')).toBe('/no/such/proton-auth');
  });
});

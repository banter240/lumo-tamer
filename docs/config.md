...
Built-in models (`server.allowedModels`): `lumo`, `lumo-lite`, `lumo-max`, and **`lumo-lead`**. `lumo-lead` is not an `extraModels` alias — it resolves to lumo-max + thinking (`reasoning: high`) with `agent: lead` (keeps orchestration tools like `task`, strips coding tools; slim `forOrchestrationTools`, never coding `forTools`). Worker announce-without-tool is bounced once via `forAnnounceBounce`. Own aliases (`server.extraModels`): `id` + `model` (`lumo` / `lumo-lite` / `lumo-max`) + optional `reasoning` (`none` / `high`) + optional `agent` (`lead` / `worker`). Example: `lumo-lite-thinking` → Lite with thinking on. Request `reasoning_effort` still wins. Optional `agent: lead` on a custom alias also uses `server.agentProfiles.lead` and strips tools; absent/`worker` (and built-in `lumo-max`) keep the coding instructions + custom-tools path.

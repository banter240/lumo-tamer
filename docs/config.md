...
- **Save and restart** and **Reset to defaults** sit in the toolbar. Save writes only overrides. Reset restores `config.defaults.yaml` except `server.apiKey` (confirm first). Both restart: Docker exits so compose can bring it back; local `tsx watch` pokes the entry file. The page waits for `/health` and reloads.
...
PUT  /v1/config   apply edits or { resetAll: true } (keeps apiKey), then restart
```
...
Common sections: `auth`, `server`, `cli`, `log`, `conversations`, `commands`. See the defaults file for every key.